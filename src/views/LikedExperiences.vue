            ...
            <div class="row">
              ...
              <div v-if="FavoritesExperience.length < 1 && !loading">
                <empty-result>
                  ...
                  You do not have any Favorite Experience yet.
                  <br />When you love an experience, it will appear here.
                </empty-result>
              </div>

              <div
                class="col-md-4 experience"
                v-for="experience in FavoritesExperience"
                :key="experience.id"
                style
              >
                <router-link
                  :to="'/experience/'+ experience.experience.id + '/' + experience.experience.slug"
                >
                  <div class="featured-card card" style="margin-bottom: 20px;">
                    <i
                      class="fa fa-heart"
                      style="position: absolute;float: right;padding: 20px;font-size: 18px;color: #FFF;color: rgb(248, 24, 148);"
                    ></i>

                    <img
                      v-if="experience.experience.images.length"
                    ...
                    <div class="card-body">
                      <p
                        class="card-text"
                        style="color: #f81894"
                      >{{ experience.experience.title }} | {{ experience.experience.state }}</p>
                      <h5
                        class="card-title"
                        style="text-transform: capitalize;"
                      >{{ experience.experience.city }}</h5>
                      <star-rating
                        :rating="parseFloat(experience.experience.rating)"
                        :read-only="true"
                        :increment="0.01"
                        :star-size="18"
                      ></star-rating>
                      <p class="card-text" style="color: #f81894">
                        <b>
                          {{experience.experience.rating == null ? 0 : experience.experience.rating}}
                          <i class="fa fa-star"></i>
                        </b>
                        ({{experience.experience.rating_count == null ? 0 : experience.experience.rating_count}})
                      </p>
                    </div>
                  </div>
                </router-link>
  ...
  methods: {
    ...
    getUserFavorites() {
      if (this.$store.state.auth) {
        this.loading = true;
        Axios.get(
          `${this.$store.state.API_BASE}/favourites?user_id=${this.$store.state.auth.user.id}`
        )
          .then(response => {
